...
example_output = """
$ vol.py -f ~/Snapshot19.vmem  --profile=Win7SP1x64 mimikatz
Volatility Foundation Volatility Framework 2.6.1
Module   User             Domain           Password                                
-------- ---------------- ---------------- ----------------------------------------
wdigest  John             John-PC                                                  
wdigest  JOHN-PC$         WORKGROUP                                                
randark@memory-forensic:~/memory-image-auto-analyzer$ 
"""


class Volatility2_debug():
...
class Volatility2_constants():
    # import volatility.constants as constants
    def SCAN_BLOCKSIZE(self):
        # import volatility.debug as debug
        # debug.warning
        pass


class Volatility2_commands():
...
class Volatility2_utils():
    # import volatility.utils as utils
    def load_as(self):
        pass


class Volatility2_tasks():
...
debug = Volatility2_debug()
# constants = Volatility2_constants()
from volatility3.framework import interfaces, constants
commands = Volatility2_commands()
utils = Volatility2_utils()
tasks = Volatility2_tasks()
...
class Credential():
    ...
    def dump(self):
        debug.notice('m<{}> u<{}> d<{}> ep<{}> p<{}>'.format(self.module,
...
class MemoryScanner(object):
    ...
    def _find_first(self, address_space, offset, maxlen, signature):
        ...
        while i < offset + maxlen:
            to_read = min(constants.SCAN_BLOCKSIZE + overlap, offset + maxlen - i)
...
class LsaDecryptor():
    ...
    HARD_KEY = construct.Struct('cbSecret' / construct.Int32ul,
    ...
    def dump(self):
        ...
        print('DES_KEY ({}): {}'.format(len(self.des_key), self.des_key.encode('hex')))
        print('AES_KEY ({}): {}'.format(len(self.aes_key), self.aes_key.encode('hex')))
    ...
    def get_unicode_string_at(self, ptr, size):
        ...
        if data:
            ...
            try:
                ...
                debug.error('[Wdigest] get_unicode_string_at() unicode error {}'.format(ee))
        ...
        if entry.usage_count:
            if entry.this_entry == found_at:
                ...
                if entry.domain_string_ptr and entry.domain_len:
                    domain = self.get_unicode_string_at(entry.domain_string_ptr,
                                                        entry.domain_max_len)
                if entry.password_encrypted_ptr and entry.password_len:
                    epwd = data = self.get_data(entry.password_encrypted_ptr,
                                                entry.password_max_len)
                if user:
...
class Wdigest_x86(Wdigest, Mimikatz_x86):
  ...
  '''
    WDIGEST_LIST_ENTRY = construct.Struct(
        'previous' / construct.Int32ul, 'next' / construct.Int32ul,
        'usage_count' / construct.Int32ul, 'this_entry' / construct.Int32ul,
        'luid' / construct.Int64ul, 'flag' / construct.Int64ul, 'user_len' / construct.Int16ul,
        'user_max_len' / construct.Int16ul, 'user_string_ptr' / construct.Int32ul,
        'domain_len' / construct.Int16ul, 'domain_max_len' / construct.Int16ul,
        'domain_string_ptr' / construct.Int32ul, 'password_len' / construct.Int16ul,
        'password_max_len' / construct.Int16ul, 'password_encrypted_ptr' / construct.Int32ul)
...
class Wdigest_x64(Wdigest, Mimikatz_x64):
  ...
  '''
    WDIGEST_LIST_ENTRY = construct.Struct(
        'previous' / construct.Int64ul, 'next' / construct.Int64ul,
        'usage_count' / construct.Int32ul, 'align1' / construct.Int32ul,
        'this_entry' / construct.Int64ul, 'luid' / construct.Int64ul, 'flag' / construct.Int64ul,
        'user_len' / construct.Int16ul, 'user_max_len' / construct.Int16ul,
        'align2' / construct.Int32ul, 'user_string_ptr' / construct.Int64ul,
        'domain_len' / construct.Int16ul, 'domain_max_len' / construct.Int16ul,
        'align3' / construct.Int32ul, 'domain_string_ptr' / construct.Int64ul,
        'password_len' / construct.Int16ul, 'password_max_len' / construct.Int16ul,
        'align4' / construct.Int32ul, 'password_encrypted_ptr' / construct.Int64ul)
...
class mimikatz(commands.Command):
    ...
    def render_text(self, outfd, data):
        self.table_header(outfd, [("Module", "8"), ("User", "16"), ("Domain", "16"),
                                  ("Password", "40")])
        for cred in self.credentials_obj.credentials:
            self.table_row(outfd, cred.module, cred.username, cred.domain, cred.pwd)
